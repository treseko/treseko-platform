...
export function ExtensionInstanceDetails({
  ...
  return <div className="d-grid gap-4">
    ...
    <section>
      ...
      <Row className="g-2">
        <Col md={7}><Form.Label className="small fw-bold text-muted">{t('configuracion.extensionServiceUrl')}</Form.Label><Form.Control name="a11y-extensioninstancedetailstsx-57" aria-label="Campo de formulario" size="sm" value={configDraft.url} placeholder="https://service.example.local" onChange={event => setConfigDraft(previous => ({ ...previous, url: event.target.value }))} disabled={!canConfigure || premiumBlocked} /></Col>
        <Col md={5}><Form.Label className="small fw-bold text-muted">{t('configuracion.extensionProjectKey')}</Form.Label><Form.Control name="a11y-extensioninstancedetailstsx-58" aria-label="Campo de formulario" size="sm" value={configDraft.project_key} placeholder="QA, APP, DEMO..." onChange={event => setConfigDraft(previous => ({ ...previous, project_key: event.target.value }))} disabled={!canConfigure || premiumBlocked} /></Col>
        <Col xs={12}><Form.Label className="small fw-bold text-muted">{t('configuracion.extensionInternalNotes')}</Form.Label><Form.Control name="a11y-extensioninstancedetailstsx-59" aria-label="Campo de formulario" as="textarea" rows={2} size="sm" value={configDraft.notes} placeholder={t('configuracion.extensionNotesPlaceholder')} onChange={event => setConfigDraft(previous => ({ ...previous, notes: event.target.value }))} disabled={!canConfigure || premiumBlocked} /></Col>
      </Row>
    ...
    <section>
      ...
      <div className="d-flex flex-column flex-md-row gap-2"><Form.Control name="a11y-extensioninstancedetailstsx-69" aria-label="Campo de formulario" size="sm" type="password" value={secretDraft} placeholder={t('configuracion.extensionSecretPlaceholder')} onChange={event => setSecretDraft(event.target.value)} disabled={!canManageSecrets || premiumBlocked} /><Button size="sm" variant="outline-primary" className="app-save-button text-nowrap" onClick={onSaveSecret} disabled={saving || !canManageSecrets || !secretDraft.trim() || premiumBlocked}>{t('configuracion.extensionSaveSecret')}</Button></div>
