...
export function WorkflowSidebar({
  ...
  return (
    <aside className="workflow-sidebar">
          ...
          <span className="d-flex gap-1">
            <Dropdown>
              ...
              <Dropdown.Menu align="end">
                ...
                <Dropdown.Item onClick={createUniversalWorkflow} disabled={!workflowDraft}>{t('configuracion.workflowUniversal')}</Dropdown.Item>
                <Dropdown.Item onClick={openUniversalAgentCreator}>{t('configuracion.createUniversalAgent')}</Dropdown.Item>
              </Dropdown.Menu>
            </Dropdown>
            <Button size="sm" variant="outline-secondary" title={t('configuracion.cloneWorkflow')} aria-label={t('configuracion.cloneWorkflow')} disabled={!workflowDraft} onClick={cloneWorkflow}><Copy size={14} /></Button>
          </span>
        )}
      </div>
      <Form.Group className="px-2 pb-2">
        <Form.Label visuallyHidden htmlFor="workflow-filter">{t('configuracion.searchWorkflowLabel')}</Form.Label>
        <div className="position-relative">
          <Search size={13} className="position-absolute text-muted" style={{ left: 9, top: 10 }} />
          <Form.Control id="workflow-filter" size="sm" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t('configuracion.searchWorkflow')} style={{ paddingLeft: 29 }} />
        </div>
      </Form.Group>
      ...
      {canEditAi && (
        <>
          <div className="workflow-panel-title mt-3"><Bot size={14} /> {t('configuracion.addAgent')}</div>
          <div className="workflow-agent-library">
            {visibleAgentPresets.map(preset => {
              ...
              return (
                <button
                  ...
                  title={workflowDraft?.status === 'ACTIVE' ? t('configuracion.activeWorkflowEditHint') : t('configuracion.dragAgentTitle', { name: preset.name })}
            ...
            {visibleAgentPresets.length === 0 && <div className="small text-muted border rounded-3 p-3">{t('configuracion.noCompatibleBlocks')}</div>}
          </div>
          <div className="workflow-pending-agent">{t('configuracion.dragAgentHint')}</div>
        </>
      )}
    </aside>
  )
