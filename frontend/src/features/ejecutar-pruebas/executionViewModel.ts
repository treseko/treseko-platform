import {
  getSuiteAndDescendantIds as getSuiteAndDescendantIdsFromTree,
...
export function filterSuiteTreeByIds(
  suites: any[],
  suiteIds: Set<string>,
): any[] {
  return suites
    .map((suite) => {
      ...
      return suiteIds.has(suite.id) || children.length > 0
        ? { ...suite, children }
        : null;
    })
    .filter(Boolean);
...
export function buildExecutionViewModel({
  ...
  const belongsToCurrentComponent = (test: any) =>
    Boolean(currentCompId) && test.componentId === currentCompId;
  const activeBuildCaseIds = currentBuildId
    ? buildCaseIds[currentBuildId] || []
    : [];
  const activeBuildCaseSet = new Set(activeBuildCaseIds);
  const activeBuildResultHistory = currentBuildId
    ? buildCaseResultHistoryByBuild[currentBuildId] || {}
    : {};
  const activeBuildResultsLoaded = currentBuildId
    ...
    : false;
  const activeBuildResultsLoading = currentBuildId
    ? Boolean(latestResultsLoadingByBuild[currentBuildId])
    : false;
  const activeBuildCasesLoading = currentBuildId
    ? Boolean(buildCasesLoadingByBuild[currentBuildId])
    : false;
  const hydrateTestWithActiveBuildResult = (test: any) => {
    if (!currentBuildId || !activeBuildCaseSet.has(test.id)) return test;
    if (
      !Object.prototype.hasOwnProperty.call(activeBuildResultHistory, test.id)
    )
      return test;
  ...
  const currentExecutionCases = currentExecutionCaseSource.filter(
    (test) =>
      executionContextReady &&
      belongsToCurrentComponent(test) &&
      activeBuildCaseSet.has(test.id),
  );
  const executionSuiteIds = new Set(
    currentExecutionCases.map((test) => test.suiteId).filter(Boolean),
  );
  const executionSuiteTree = executionContextReady
    ? filterSuiteTreeByIds(suitesTree, executionSuiteIds)
    : [];
  const executionInitialLoading =
    (suitesLoading || casosLoading || activeBuildCasesLoading) &&
    executionSuiteTree.length === 0 &&
    currentExecutionCases.length === 0;
  const executionRefreshing =
    (suitesLoading ||
      casosLoading ||
  ...
  const filteredTests = orderTestsBySuiteTree(
    currentExecutionCaseSource.filter((test) => {
      const belongsToComponent = belongsToCurrentComponent(test);
      const belongsToCurrentBuild =
        currentBuildId && activeBuildCaseSet.has(test.id);
      if (query) {
        return (
          belongsToComponent && belongsToCurrentBuild && matchesCaseQuery(test)
        );
      }
      return (
        belongsToComponent &&
  ...
  const suiteCandidateTests = currentExecutionCaseSource.filter(
    (test) =>
      (selectedUnsuitedCases
        ? !test.suiteId
        : selectedSuiteAndDescendantIds.includes(test.suiteId)) &&
  ...
  const suiteComponentMismatchCount = suiteCandidateTests.filter(
    (test) => !belongsToCurrentComponent(test),
  ).length;
  const suiteBuildMissingCount = suiteCandidateTests.filter(
    (test) =>
      belongsToCurrentComponent(test) && !activeBuildCaseSet.has(test.id),
  ).length;

  const executionSelectableTests = currentExecutionCaseSource.filter(
    (test) =>
      belongsToCurrentComponent(test) && activeBuildCaseSet.has(test.id),
  );
  const allExecutionTestsById = new Map(
    currentExecutionCaseSource.map((test) => [test.id, test]),
  );
  const executionTestsById = new Map(
    executionSelectableTests.map((test) => [test.id, test]),
  );
  const resolveExecutionCandidate = (testId: string) => {
    ...
    if (
      candidate &&
      belongsToCurrentComponent(candidate) &&
      isOutdatedExecutionCase(candidate)
    )
      return candidate;
    return null;
  };
  ...
  const selectedExecutionTests = selectedExecutionTestIds
    ...
    .filter(Boolean);
  const selectedExecutionDiscardedCount =
    selectedExecutionTestIds.length - selectedExecutionTests.length;
  ...
  const activeExecutionTests = (
    activeExecutionCaseIds.length > 0
      ? activeExecutionCaseIds
      : selectedExecutionTestIds
  )
  ...
  const allVisibleExecutionTestsSelected =
    filteredTests.length > 0 &&
    filteredTests.every((test) => selectedExecutionTestIds.includes(test.id));

  const getSuiteExecutionMetrics = (suiteId: string) => {
    const unsuitedCases = suiteId === UNSUITED_CASES_ROOT_ID;
    const suiteIds = unsuitedCases
      ? []
      : getSuiteAndDescendantIdsFromTree(suitesTree, suiteId);
    const suiteTests = currentExecutionCaseSource.filter(
      (test) =>
    ...
    return {
      ...
      passed: suiteTests.filter(
        (test) => getExecutionStatusKey(test) === "passed",
      ).length,
      failed: suiteTests.filter(
        (test) => getExecutionStatusKey(test) === "failed",
      ).length,
      blocked: suiteTests.filter(
        (test) => getExecutionStatusKey(test) === "blocked",
      ).length,
      pending: suiteTests.filter(
  ...
  const getExecutionActionLabel = (test: any) => {
    if (!isOutdatedExecutionCase(test))
      return `Ejecutar ${getExecutionCaseLabel(test)}`;
