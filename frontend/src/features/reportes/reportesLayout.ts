...
export const REPORTES_ROW_HEIGHT = 56
...
const REPORTES_DEFAULT_LAYOUT: LayoutItem[] = [
  ...
  { i: 'context', x: 0, y: 5, w: 12, h: 2, minW: 4, minH: 2 },
...
export const withReportesEditFlags = (layouts: ResponsiveLayouts<string>, editing: boolean): ResponsiveLayouts<string> =>
  Object.fromEntries(Object.entries(layouts).map(([breakpoint, layout]) => [
    breakpoint,
    (layout || []).map((item) => ({
      ...sanitizeReportesLayoutItem(item as LayoutItem & Record<string, any>),
      isDraggable: editing,
      isResizable: editing,
      isBounded: true,
      resizeHandles: ['se'],
...
export const stripReportesEditFlags = (layouts: ResponsiveLayouts<string>): ResponsiveLayouts<string> =>
  Object.fromEntries(Object.entries(layouts).map(([breakpoint, layout]) => [
    breakpoint,
    (layout || []).map((item) => sanitizeReportesLayoutItem(item as LayoutItem & Record<string, any>)),
  ]))
