...
const traceabilityUi = {
  ...
  en: {
    ...
    recommendedFormat: "Recommended format", acceptanceHint: "Define verifiable behavior. These criteria remain linked to test cases and enable assisted generation.", context: "Context", analysis: "Analysis", configuration: "Configuration", review: "Review", completed: "completed", draft: "Draft", includeCreation: "Include in creation", quality: "Quality", externalReferenceLabel: "External reference", historyInitial: "Initial version", viewChanges: "View changes", historyNoPrevious: "No previous version to compare.", modificationLabel: "Modification:", noChangeCommentText: "No change comment.", noDifferencesText: "No differences detected from the previous version.", previousLabel: "Previous", currentLabel: "Current", oldestVersionText: "This is the oldest version; there is no previous version to compare.", linkedStoriesEmpty: "This requirement has no linked stories yet.", closeModal: "Close", qualityLabelText: "Quality: {label}.", reviewScopeText: "Review whether this proposal adds a different scope before selecting it. The final decision is yours.", criticalOverrideText: "I understand the critical observations and choose to create this draft.", decisionJustificationText: "Decision justification", qualityReasonPlaceholder: "Explain why this proposal should be created despite the observations.", titleLabel: "Title", priorityLabel: "Priority", descriptionLabel: "Description", acceptanceLabel: "Acceptance criteria", proposalCovered: "proposal is already covered", proposalsCovered: "proposals are already covered", existingLabel: "existing:", allScopeCoveredText: "All scope proposals are already covered by active stories. You can return to the analysis if the requirement needs a different capability.", analyzing: "Analyzing...", analyzeRequirement: "Analyze requirement", continueAssumptions: "Continue with working assumptions", continueScope: "Continue and calculate scope", draftCount: "Number of drafts", proposalCount: "Proposals", createDrafts: "Create drafts",
    editStoryTitle: "Edit story", newStoryTitle: "New story", userStory: "User story", userStoryExample: "As a [role or user], I want [action or capability] so that [benefit or value].", acceptanceMarkdown: "Acceptance criteria Markdown", structuredAcceptance: "Structured acceptance criteria", externalProvider: "External provider", externalUrl: "External URL", stages: "Generation stages", processing: "Processing", aiWorking: "AI is working", contextForAi: "Context for AI", collapseContext: "Collapse context", expandContext: "Expand context", optionalInstructions: "Optional instructions", instructionsPlaceholder: "Focus, exclusions, or criteria to consider.", contextComponents: "Context components", optionalWiki: "Optional Wiki pages", readyContext: "Context ready to continue", incompleteContext: "Incomplete context: you can continue with assumptions", blockedAnalysis: "Analysis blocked", hideExplanation: "Hide explanation", showExplanation: "View explanation", ambiguities: "Ambiguities:", proposedAssumptions: "Proposed assumptions:", answerContext: "Answers to complete the context", answerPlaceholder: "Optional: leave empty if QA does not have this information.", suggestedStories: "Suggested stories", generatedDrafts: "Generated drafts", noCompleteDraft: "There is no complete draft to show yet.", noNewDrafts: "No new drafts were generated", insertionPreview: "Insertion preview", proposal: "Proposal", userStoryType: "User story", technicalStory: "Technical story", coveredIntent: "Intent already covered", similarStory: "Similar existing story", hideDetail: "Hide detail", viewEditDetail: "View and edit detail", viewDetail: "View detail", excludeCreation: "Exclude from creation", includeCritical: "Include and review critical observations", includeCriticalAria: "Include proposal requiring review", excludeCriticalAria: "Exclude proposal requiring review", rulesPass: "The proposal meets the automatic rules.", equivalentIntent: "AI detected an equivalent functional intent.", recommendedFormatLabel: "Recommended format", storyExample: "As a quality analyst, I want to consult the execution summary so that I can identify risks before releasing a build.", acceptanceHintShort: "Define verifiable behavior. These criteria remain linked to test cases and enable assisted generation.", casesCreatedTitle: "Cases created", casesCreatedMessage: "{count} manual cases were created and traced for review.", saving: "Saving...", criticalAssumptions: "Critical assumptions require this explicit confirmation.", pauseAuto: "Pause automatic continuation", resumeAuto: "Resume automatic continuation", updating: "Updating...", backAnalysis: "Back to analysis", generating: "Generating...", creating: "Creating...", confirmReview: "Confirm and justify proposals requiring review.",
  }
} as const
...
const ReviewPendingIcon = ({ count, tooltipId }: { count: number; tooltipId: string }) => {
  ...
  const message = interpolateTraceability(
    pending === 1
      ? (locale === 'en' ? t('proyectos.pendingReview') : traceabilityUi.es.pendingReview)
      : (locale === 'en' ? t('proyectos.pendingReviews') : traceabilityUi.es.pendingReviews),
    { count: pending },
  );
...
async function readJson(response: Response) {
  ...
  if (!response.ok)
    throw new Error(payload?.detail || String(response.status));
  return payload;
}
...
export function TraceabilityTab({
  ...
  const tx = (key: TraceabilityTextKey, params?: Record<string, string | number>) => {
    const localValue = traceabilityUi[locale][key]
    const translatedValue = locale === 'en' && !localValue ? t(`proyectos.${key}` as any, params) : localValue
    return interpolateTraceability(translatedValue || key, params)
  }
