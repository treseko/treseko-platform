...
import { CheckCircle2, ChevronLeft, ChevronRight, GitBranch, PlayCircle, Rocket, ShieldCheck, Sparkles } from 'lucide-react'
import { API_BASE, TRESEKO_TELEMETRY_ENDPOINT } from '../../app/constants'
import { useI18n } from '../../i18n'
import { ADMIN_GUIDE_OPEN_EVENT, requestAdminGuideAction } from './onboardingEvents'
import { GUIDE_STEPS, GUIDE_STRUCTURE_NODES } from './onboardingGuide'
...
export function FirstRunOnboarding({ loggedUser, fetchWithAuth, onPreferencesUpdated, firstRunState, onFirstRunCompleted, systemEdition, disabled = false }: FirstRunOnboardingProps) {
  ...
  const [guideMode, setGuideMode] = useState<'onboarding' | 'help'>('onboarding')
  ...
  useEffect(() => {
    if (disabled) return
    if (firstRunState?.requires_onboarding === true && stage === 'done' && !surveyCompletedLocally) {
      setStage('survey')
      return
    }
  ...
  }, [disabled, firstRunState?.requires_onboarding, stage, surveyCompletedLocally])

  useEffect(() => {
    if (disabled || loggedUser?.role !== 'ADMIN') return
    const handleGuideRequest = () => {
      if (firstRunState?.requires_onboarding === true) return
      setGuideMode('help')
      setGuideIndex(0)
      setError('')
      setStage('guide')
    }
    window.addEventListener(ADMIN_GUIDE_OPEN_EVENT, handleGuideRequest)
    return () => window.removeEventListener(ADMIN_GUIDE_OPEN_EVENT, handleGuideRequest)
  }, [disabled, firstRunState?.requires_onboarding, loggedUser?.role])
  ...
  const closeGuide = async () => {
    ...
    setError('')
    try {
      if (guideMode === 'onboarding' && !hasSeenGuide(profileSettings)) await patchProfileSettings({
        ...profileSettings,
  ...
  const guideCopy = {
    productStructure: {
      ...
      text: t('onboarding.structureText'),
      fragmented: t('onboarding.structureFragmented'),
      treseko: t('onboarding.structureTreseko'),
      chips: [t('onboarding.guideSolutions'), t('onboarding.guideProjects'), t('onboarding.guideSuites')],
    },
    connectedQuality: {
      title: t('onboarding.valueConnectedTitle'),
      text: t('onboarding.valueConnectedText'),
      fragmented: t('onboarding.valueConnectedFragmented'),
      treseko: t('onboarding.valueConnectedTreseko'),
      chips: [t('onboarding.classicTests'), t('onboarding.apiTests'), t('onboarding.conversationalTests')],
    },
    evidenceToBug: {
      title: t('onboarding.valueEvidenceTitle'),
      text: t('onboarding.valueEvidenceText'),
      fragmented: t('onboarding.valueEvidenceFragmented'),
      treseko: t('onboarding.valueEvidenceTreseko'),
      chips: [t('onboarding.realEvidence'), t('onboarding.traceableBugs'), t('onboarding.incidentHistory')],
    },
    releaseReports: {
      title: t('onboarding.valueReportsTitle'),
      text: t('onboarding.valueReportsText'),
      fragmented: t('onboarding.valueReportsFragmented'),
      treseko: t('onboarding.valueReportsTreseko'),
      chips: [t('onboarding.currentBuild'), t('onboarding.pendingHistory'), t('onboarding.shareableSnapshot')],
    },
  }
  const translatedGuide = guideCopy[currentGuide.key]
  ...
  const selectedTools = answers.current_tools.map(labelOption)
  const marketInsight = answers.current_tools.length > 1
    ? t('onboarding.marketMultipleTools')
    : answers.current_tools.includes('Excel / Sheets')
      ? t('onboarding.marketSpreadsheet')
      : answers.current_tools.includes('Jira') || answers.current_tools.includes('GitHub Issues')
        ? t('onboarding.marketIssueTracker')
        : answers.current_tools.includes('TestRail / Zephyr / Xray')
          ? t('onboarding.marketTestManagement')
          : answers.current_tools.includes('Herramientas de automatizacion')
            ? t('onboarding.marketAutomation')
            : t('onboarding.marketInformal')
        ...
        <>
          <Modal.Header className="border-0 pb-0">
            <div className="d-flex align-items-center gap-3">
              <div className="onboarding-icon-badge"><Sparkles size={22} aria-hidden="true" /></div>
              <div>
          ...
          <Modal.Footer className="border-0 pt-0">
            <Button variant="primary" className="fw-bold rounded-pill px-4" disabled={!surveyValid || saving} onClick={saveSurvey}>
              {saving ? <Spinner size="sm" className="me-2" aria-hidden="true" /> : <CheckCircle2 size={16} className="me-2" aria-hidden="true" />}
          ...
          <Modal.Header className="border-0 pb-0">
            <div className="d-flex align-items-center justify-content-between w-100 gap-3">
              <div className="d-flex align-items-center gap-3 min-w-0">
                <div className="onboarding-icon-badge"><Rocket size={22} aria-hidden="true" /></div>
            ...
            <div className="onboarding-guide-layout">
              <div className="onboarding-flow" aria-hidden="true">
                {GUIDE_STEPS.map((step, index) => {
                  ...
                  return (
                    <div key={step.key} className={`onboarding-flow-node ${index === guideIndex ? 'is-active' : ''} ${index < guideIndex ? 'is-done' : ''}`}>
                      <div className="onboarding-flow-dot"><StepIcon size={18} aria-hidden="true" /></div>
                      <div className="onboarding-flow-label">{guideCopy[step.key].title}</div>
                    </div>
                  )
                })}
              </div>
              <Card className="onboarding-guide-card">
                <Card.Body>
                  <div className="d-flex align-items-start gap-3">
                    <div className="onboarding-guide-icon"><GuideIcon size={34} aria-hidden="true" /></div>
                    <div className="min-w-0">
                      <div className="small fw-bold text-primary text-uppercase onboarding-eyebrow">{t('onboarding.whatMakesUsDifferent')}</div>
                      <h3 className="fw-bold text-dark mt-1 mb-2">{translatedGuide.title}</h3>
                  ...
                  <div className="onboarding-difference-grid mt-4">
                    <div className="onboarding-difference-card">
                      <div className="onboarding-difference-label">{t('onboarding.whenToolsAreSeparate')}</div>
                      <p>{translatedGuide.fragmented}</p>
                    </div>
                    <div className="onboarding-difference-card is-treseko">
                      <div className="onboarding-difference-label"><GitBranch size={15} aria-hidden="true" /> {t('onboarding.withTreseko')}</div>
                      <p>{translatedGuide.treseko}</p>
                    </div>
                  </div>
                  <div className="onboarding-current-flow mt-3">
                    <div className="small fw-bold text-dark">{t('onboarding.yourStartingPoint')}</div>
                    <div className="small text-muted text-break">
                      {selectedTools.length > 0 && <strong>{selectedTools.join(' + ')}: </strong>}{marketInsight}
                    </div>
                  </div>
                  {currentGuide.key === 'productStructure' ? (
                    <div className="onboarding-structure mt-3" aria-label={t('onboarding.structureAria')}>
                      <div className="small fw-bold text-dark mb-2">{t('onboarding.structureHint')}</div>
                      <div className="onboarding-tree">
                        {GUIDE_STRUCTURE_NODES.map(({ key, action, level, icon: NodeIcon }) => (
                          <button
                            key={key}
                            type="button"
                            className="onboarding-tree-node"
                            style={{ '--onboarding-tree-level': level } as CSSProperties}
                            data-onboarding-action={action}
                            onClick={() => requestAdminGuideAction(action)}
                          >
                            <NodeIcon size={16} aria-hidden="true" />
                            <span className="onboarding-tree-label">
                              <strong>{t(`onboarding.structure${key[0].toUpperCase()}${key.slice(1)}`)}</strong>
                              <small>{t(`onboarding.structure${key[0].toUpperCase()}${key.slice(1)}Text`)}</small>
                            </span>
                            <span className="onboarding-tree-action">{t('onboarding.openSection')}</span>
                          </button>
                        ))}
                      </div>
                    </div>
                  ) : (
                    <div className="onboarding-hierarchy mt-3" aria-label={t('onboarding.connectedFlow')}>
                      <span>{t('onboarding.cases')}</span>
                      <span>{t('onboarding.execution')}</span>
                      <span>{t('onboarding.evidence')}</span>
                      <span>{t('onboarding.bug')}</span>
                      <span>{t('onboarding.reports')}</span>
                    </div>
                  )}
                  <ProgressBar
                    now={((guideIndex + 1) / GUIDE_STEPS.length) * 100}
                    className="mt-4 onboarding-progress"
                    aria-label={t('onboarding.guideProgress', { current: guideIndex + 1, total: GUIDE_STEPS.length })}
                  />
                </Card.Body>
              </Card>
              <Alert variant="info" className="small border-0 onboarding-tip" role="status">
                <ShieldCheck size={16} className="me-1" aria-hidden="true" />
                {guideMode === 'help' ? t('onboarding.adminGuideTip') : t('onboarding.guideTip')}
              </Alert>
            </div>
          </Modal.Body>
          <Modal.Footer className="border-0 pt-0 d-flex justify-content-between">
            <div className="d-flex align-items-center gap-2">
              <Button variant="outline-secondary" className="fw-bold rounded-pill" disabled={guideIndex === 0 || saving} onClick={() => setGuideIndex(index => Math.max(0, index - 1))}>
                <ChevronLeft size={16} aria-hidden="true" /> {t('onboarding.previous')}
              </Button>
              {guideMode === 'onboarding' && (
                <Button variant="link" className="text-muted fw-semibold text-decoration-none" disabled={saving} onClick={closeGuide}>
            ...
            <Button
              ...
              disabled={saving}
              onClick={() => isLastGuideStep ? closeGuide() : setGuideIndex(index => Math.min(GUIDE_STEPS.length - 1, index + 1))}
            >
              {saving ? <Spinner size="sm" className="me-2" aria-hidden="true" /> : isLastGuideStep ? <PlayCircle size={16} className="me-2" aria-hidden="true" /> : null}
              {isLastGuideStep ? guideMode === 'help' ? t('onboarding.closeHelp') : t('onboarding.start') : t('onboarding.next')}
              {!isLastGuideStep && <ChevronRight size={16} className="ms-1" aria-hidden="true" />}
            </Button>
          </Modal.Footer>
        </>
      )}
