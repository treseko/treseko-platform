...
export default function App() {
  ...
  const contextHydrationVersionRef = useRef(0);
  const { handleOrgChange, handleProjectChange, handleComponentChange, hydrateProjectContext, refreshCurrentTestContext, loadProjectTestContext } = createContextActions({ activeTab,projectsSource,managingProjectId,contextHydrationVersionRef,currentProjectId,currentCompId,currentBuildId,projectsList,componentsList,loadComponentsForProject,loadBuildsForProject,loadSuitesFromBackend,loadCasosFromBackend,loadBuildCases,loadBuildCaseExecutionStatus,setCurrentOrgId,setManagingProjectId,setSelectedTest,setSelectedExecutionTestIds,setSelectedTestsForIa,setProjectMetrics,setCurrentProjectId,setCurrentCompId,setCurrentBuildId,setViewMode,setNewTestComponent });
  ...
  useEffect(() => {
    ...
    // The project administration screen is scoped by managingProjectId. Using
    // currentProjectId here can rehydrate the previous project immediately
    // after a component click and restore its old component/build selection.
    const contextProjectId = activeTab === "proyectos" && projectInnerTab === "components"
      ? managingProjectId
      : currentProjectId;
    if (!contextProjectId) return;
    hydrateProjectContext(contextProjectId, currentCompId);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    activeTab,
    projectInnerTab,
