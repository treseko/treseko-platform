# Treseko Community documentation

<!-- Language: en -->

> Español: [Documentación en español](../README.md)

This documentation helps you install Treseko, organize QA work and
administer the platform. Each guide explains which feature is available, how to
use it and what to check if something does not behave as expected.

## Getting started

1. [Quick installation](INSTALLATION.md): install Treseko with Docker or from your machine via SSH.
2. [Docker guide](DOCKER_GUIDE.md): set up a persistent installation.
3. [Access, users and permissions](AUTH_RBAC_GUIDE.md): create the first users and roles.
4. [Suite and case portability](CASE_PORTABILITY.md): import or export your cases.
5. [Linux installation](LINUX_SETUP.md): prepare a Linux installation step by step.

## Using Treseko

- [Project guide](PROJECTS_GUIDE.md)
- [Dashboard](DASHBOARD_GUIDE.md)
- [Create and maintain test cases](TEST_CASES_GUIDE.md)
- [Execute tests](TEST_EXECUTION_GUIDE.md)
- [Inventory](INVENTORY_GUIDE.md)
- [Reports and metrics](REPORTING_GUIDE.md)
- [Run history](RUN_HISTORY_GUIDE.md)
- [Execution states](EXECUTION_STATES.md)
- [Project states](PROJECT_STATUS_RESTRICTIONS.md)
- [Bug Tracker](BUG_TRACKER.md)
- [Attachments and evidence](ATTACHMENTS_EVIDENCE.md)
- [Email notifications](NOTIFICATIONS_EMAIL.md)
- [Traceability and assisted generation](TRACEABILITY.md)
- [Import compatibility](CASE_IMPORT_COMPATIBILITY.md)

## Automation and AI

- [Automation](AUTOMATION_GUIDE.md)
- [External automation: generate an API key and report executions](API_USAGE_GUIDE.md)
- [External automation API contract](EXTERNAL_AUTOMATION_API.md)
- [API specification](API_SPEC.md)
- [Automation worker](AUTOMATION_WORKER_V1.md)
- [Running the AI Engine](AI_ENGINE_GUIDE.md)
- [AI Engine configuration](AI_ENGINE_CONFIG.md)
- [Governed MCP](MCP_GUIDE.md)

## Administration and reference

- [Platform configuration](SETTINGS_GUIDE.md)
- [Community and Premium editions](EDITION_STRATEGY.md)
- [Integrations and plugins](INTEGRATIONS_PLUGIN_RBAC.md)
- [Permission matrix by role](RBAC_MATRIX.md)
- [Capability matrix](RBAC_CAPABILITY_MATRIX.md)
- [Architecture](ARCHITECTURE.md)
- [Recommended distributed deployment](DISTRIBUTED_DEPLOYMENT_GUIDE.md)
- [Database](DATABASE.md)

## Distributed version

The distributed candidate version is `1.0.1-rc.4`. It includes operational improvements in installation,
updates, case portability, traceability, reports, automation,
AI and administration.

See the [changelog](../../CHANGELOG.md) for the included changes.

## Official links

- Website: [treseko.com](https://treseko.com)
- [Terms and conditions](https://treseko.com/en/terms-and-conditions)
