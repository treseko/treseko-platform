...
from .recipient_resolver import resolve_recipients, user_allows_channel, recipient_frequency, recipient_schedule
...
DEFAULT_TEMPLATE_VARIABLES = [
    ...
    "user.auth_provider", "platform.name", "message",
]

# These categories are presentation metadata only.  They deliberately do not
# decide recipients: audience remains governed by each persisted rule.
ADMINISTRATIVE_EVENT_TYPES = {
    "user.created", "user.disabled", "user.role_changed", "role.permissions_changed",
    "project.member_added", "project.member_removed", "build.activated", "build.closed",
}
SECURITY_EVENT_TYPES = {"auth.login_failed_many", "evidence.required_missing", "automation.runner.offline"}
...
SEED_TEMPLATES = [
    ...
    {
        "key": "welcome_local_email",
        "nombre": "Bienvenida usuario local",
        "channel": "email",
        "subject_template": "Bienvenido/a a ${platform.name}",
        "text_template": "${message}",
    },
    {
        "key": "welcome_sso_email",
        "nombre": "Bienvenida acceso corporativo",
        "channel": "email",
        "subject_template": "Acceso corporativo a ${platform.name}",
        "text_template": "${message}",
    },
    {
        "key": "digest_email",
        "nombre": "Resumen programado",
        "channel": "email",
        "subject_template": "[${proyecto.nombre}] Resumen de Treseko",
        "text_template": "${message}",
    },
    {
        "key": "build_closed_email",
        "nombre": "Cierre de build",
        "channel": "email",
        "subject_template": "Build cerrada: ${build.nombre}",
        "text_template": "${message}\n\nProyecto: ${proyecto.nombre}",
    },
    {
        "key": "project_closed_email",
        "nombre": "Cierre de proyecto",
        "channel": "email",
        "subject_template": "Proyecto cerrado: ${proyecto.nombre}",
        "text_template": "${message}",
    },
]
...
SEED_RULES = [
    ...
    {
        "nombre": "Reporte exportado",
        "event_types": ["report.exported"],
        "conditions_json": {},
        "actions_json": {"channels": ["in_app", "email"], "frequency": "on_report_export"},
        "recipient_strategy_json": {"project_roles": ["QA_LEAD"], "global_roles": ["ADMIN"], "stakeholders": True},
        "template_key": "report_shared_email",
        "priority": 91,
    },
    {
        "nombre": "Cierre de build",
        "event_types": ["build.closed"],
        "conditions_json": {},
        "actions_json": {"channels": ["in_app", "email"], "frequency": "on_build_closure"},
        "recipient_strategy_json": {"project_roles": ["QA_LEAD"], "global_roles": ["ADMIN"], "stakeholders": True},
        "template_key": "project_closed_email",
        "priority": 94,
    },
    {
        "nombre": "Cierre de proyecto",
        "event_types": ["project.closed", "project.reopened"],
        "conditions_json": {},
        "actions_json": {"channels": ["in_app", "email"], "frequency": "on_project_closure"},
        "recipient_strategy_json": {"project_roles": ["QA_LEAD"], "global_roles": ["ADMIN"], "stakeholders": True},
        "template_key": "project_closed_email",
        "priority": 95,
    },
