...
from ...services.auth_ad import ldap_service, user_sync_service
...
async def create_usuario_admin(
    ...
    if user.auth_provider.lower() == "local":
        if not user.password and not user.send_welcome:
    ...
    if user.send_welcome:
        invitation = await welcome_service.queue_welcome(db, user=new_user, actor_id=current_user.id)
        await db.commit()
        await crud.create_audit_log(
...
@router.post("/usuarios/{usuario_id}/welcome/revoke/")
async def revoke_user_welcome_invitation(
    request: Request,
    usuario_id: UUID,
    db: AsyncSession = Depends(get_db),
    current_user: models.Usuario = Depends(auth.check_capability("configuracion.usuarios", "edit")),
    ...
    invitation = (await db.execute(
        select(models.NotificationWelcomeInvitation)
        .filter(models.NotificationWelcomeInvitation.user_id == target_user.id)
        .filter(models.NotificationWelcomeInvitation.used_at.is_(None))
        .filter(models.NotificationWelcomeInvitation.revoked_at.is_(None))
        .order_by(models.NotificationWelcomeInvitation.created_at.desc())
    )).scalars().first()
    if not invitation:
        raise HTTPException(status_code=404, detail="No hay una invitación activa para revocar")
    invitation.revoked_at = utc_now()
    await db.commit()
    await crud.create_audit_log(
        db=db, usuario_id=current_user.id, accion="REVOKE", recurso="notification_welcome", recurso_id=invitation.id,
        detalles={"user_id": str(target_user.id)}, ip_address=request.client.host if request.client else "unknown",
    )
    return {"ok": True}
